...
export const register = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (password.length < 8 || password.length > 16) {
      return next(
        new ErrorHandler("Password must be between 8 and 16 characyers", 400)
      );
    }
...
export const resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (req.body.password !== req.body.confirmPassword) {
    return next(
      new ErrorHandler("Password & confirm Passsword do not match", 400)
    );
  }
...
export const updatePassord = catchAsyncErrors(async (req, res, next) => {
